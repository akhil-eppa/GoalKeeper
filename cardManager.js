...
var btn_d_ids = new Array(cardsList.lenth);
...
function removeCard(e) {
    ...
    let opt = confirm("You are deleting the Goal: " + cardsList[(Number(btn_id[btn_id.length - 1]) - 1)].getElementsByClassName("card-title")[0].textContent + "\n" + "Are you sure you want to do this?");
    //find the number in the id of the delete button using, btn_id[btn_id.length - 1]
    if(opt == true)
        cardDiv.removeChild( cardsList[(Number(btn_id[btn_id.length - 1]) - 1)] );
    //console.log(cardsList[(Number(btn_id[btn_id.length - 1]) - 1)]);
